...
残しておきたいプリセットが複数できた場合は、ペダル本体には残しておけないので、「リコールシート」に目盛りを手書きメモするか、「UAFX Control モバイル・アプリ」の「User」領域に保存しておけば、ペダル本体のプリセットスロットを別のプリセットで上書きしても、のちにアプリから再度ロードすることができる。  
### 1-2.チャンネルについて
