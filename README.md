...
「プリセットモード」で呼び出せる音色（プリセットスロットに保存できるプリセット）は１つだけであり、別のプリセットに切り替えるということはできない。  
残しておきたいプリセットが複数できた場合は、ペダル本体には1つしか残しておけないので、「リコールシート」に目盛りを手書きメモして残しておくか、もしくは、「UAFX Control モバイル・アプリ」の「User」領域に保存しておけば、のちにアプリから再度ロードすることができる。  
しかし、「このペダルはアンプなのである」とみなせば、「ライブモードで使用し、つまみは都度0に戻して、演奏の際にいいポジションまで回す。演奏中には切り替えない。」ぐらいの運用でもよいのかもしれない。  
なお、スマホのアプリからの設定で、フットスイッチに「BOOST」や「VIBRATO」のON/OFFを割り当てることもできる。  
### 1-2.チャンネルについて
...
再ダウンロードできるもの（ボーナスエフェクト（Rubyではスピーカー3つ）やプリセット）はバックアップできないし、する必要もない。  
しかし、モバイルバッテリーで起動した場合（出力が足りなかった？）などに、動作がおかしくなって音が出なくなってしまうことがあり、４、５回初期化を繰り返さないと復旧しなかった。  
UAFX ペダルをデフォルト設定に戻すと、保存されていたプリセット、ボーナスエフェクト、および製品登録が削除されるので、スマホとのペアリングや製品登録やボーナスエフェクトのダウンロード等、すべてやり直す必要がある。  
スマホと再接続する場合、まえもって、「UAFX Control モバイルアプリ」からペダルのアイコンをいったん削除し（左にスワイプすると削除ボタンが出てくる）、さらにスマホの Bluetooth 設定の接続済みデバイスからもペダルをいったん削除しておく必要がある。  
...
4. 完全に起動したら、さらにUAFX ペダルの電源を抜いて電源を切り、再度電源を入れる。この2度目の再起動後、ペダルが使用できるようになる。  
