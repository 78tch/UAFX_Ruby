...
3. 「VIB」チャンネルだけで使うノブ：altにして「SPEED」、「INTENSITY」（INTENSITYを0にすると、OFFになる。）  
### 1-5.音作り手順概要
1. 3チャンネルそれぞれ、まずは各ノブ12時から始める。
2. 「VOLUME」を、ノイズが気にならない限界まで上げる。
3. 「BOOST」を0からMAXまで回して、そのチャンネルで心地よい所を探す。
4. 「OUTPUT」を、ノイズが気にならない限界まで上げる。
5. 「CUT」（Brilはさらに「BASS」、「TREBLE」）で、音質を微調節する。
6. 「ROOM」やビブラートを調節する。
7. 「VOLUME」を下げて「OUTPUT」を上げて、より心地よい音になるか試す。
8. 以上を適宜繰り返す。
