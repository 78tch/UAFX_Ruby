...
フットスイッチで、ノブの目盛り状態のとおりの音がでる「ライブモード」と、ノブの目盛りに関わらず、保存した設定の音が出る「プリセットモード」との、２つのモードを切り替えながら使う。  
なお、プリセットスロットに保存できるプリセットは１つだけである。  
残しておきたいプリセットが複数できた場合は、ペダル本体には残しておけないので、「リコールシート」に目盛りを手書きメモするか、もしくは、「UAFX Control モバイル・アプリ」の「User」領域に保存しておけば、ペダル本体のプリセットスロットを別のプリセットで上書きしても、のちにアプリから再度ロードすることができる。  
### 1-2.チャンネルについて
...
3. VIBRATOチャンネル：63年製の「トップ・ブースト」、ビブラートは速さや深さを調整でき、切ることもできるし、設定によっては、フットスイッチでビブラートのON/OFFをすることもできる。“BOOST” は、クリーンなゲイン。  
### 1-3.スピーカーについて
...
14. 「Alt/Amp/Store」スイッチをStoreに0.5秒以上倒すことで、いまの設定がペダル本体のプリセットスロットに保存され、「UAFX Control モバイル・アプリ」でつないでいると、「User」領域にも保存される。
...
「このペダルはアンプなのである」、とみれば、「保存などせず、つまみは都度0に戻して、都度いいポジションまで回す。目盛りにしるしを付ける。」ぐらいの運用でもよいのかもしれない。  
ボーナスエフェクト（Rubyではスピーカー3つ）も、アプリ側ではなくペダル本体側に保存されるのではあるが、再セットアップ時に再ダウンロードできるので、バックアップする必要はない。  
UAFX ペダルをデフォルト設定に戻すと、保存されていたプリセット、ボーナスエフェクト、および製品登録が削除される。  
初期化すると、スマホとのペアリング、製品登録やボーナスエフェクトのダウンロード等、すべてやり直す必要がある。  
スマホと再接続する場合、まえもって、UAFX コントロールからペダルのアイコンをいったん削除し（左にスワイプすると削除ボタンが出てくる）、さらにスマホの Bluetooth 設定からもペダルをいったん削除する必要があります。  
また、初期化は時間がかかるため、慌てると初期化に失敗することがある。  
初期化に失敗するとスマホから認識されなくなったり音が出なくなったりするので、その場合は再初期化すること。  
...
5. 「UAFX Control モバイルアプリ」でペダルを左にスワイプして削除ボタンを押し、Bluetoothペアリングをいったん削除したうえで、ペアリングをやり直し、製品登録などする。うまくいかない場合、一度アプリからログオフしてペダルの情報を消し、再ログインしてから試す。それでもペアリングできない場合、音が出るか確認し、音が出ない場合は初期化に失敗している可能性があるので、再度初期化する。  
### 3-3.フットスイッチの働き設定
