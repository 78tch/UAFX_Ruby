...
２つあるフットスイッチは、初期設定では左が「ライブモード」のON/OFF、右が「プリセットモード」のON/OFFが割り当てられているが、「BOOST」や「VIBRATO」のON/OFFを割り当てることもできる。
なお、プリセットスロットに保存できるプリセットは１つだけである。  
残しておきたいプリセットが複数できた場合は、ペダル本体には1つしか残しておけないので、「リコールシート」に目盛りを手書きメモするか、もしくは、「UAFX Control モバイル・アプリ」の「User」領域に保存しておけば、ペダル本体のプリセットスロットを別のプリセットで上書きしても、のちにアプリから再度ロードすることができる。  
### 1-2.チャンネルについて
