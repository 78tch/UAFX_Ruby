...
9. 以上を適宜繰り返す。
10. 設定が固まったら、「リコールシート」に目盛りをメモる。
11. 「Alt/Amp/Store」スイッチをStoreに0.5秒以上倒すことで、いまの設定がプリセットスロットに保存され、「UAFX Control モバイル・アプリ」でつないでいると、「User」領域にも保存され、別の設定でプリセットを上書きしても、アプリから再度ロードすることができる。
